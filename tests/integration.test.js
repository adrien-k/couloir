...
const setup = async (
  ...
  try {
    ...
    if (process.env.LOG !== "true" && logs.length && !expectFailure) {
      // eslint-disable-next-line no-console
      console.error(logs);
    }
    await close();
...
describe("can protect the relay with a password", () => {
  ...
  it("reject when no password", async () => {
    try {
      await setup({ expectFailure: true }, async () => {});
...
describe("with a control API", () => {
  ...
  it("can proxy an http request", async () => {
    await setup({}, async () => {
  ...
  it("count bytes and interrupts when over quota", async () => {
    await setup({}, async () => {
      ...
      await waitUntil(() => assert.equal(controlServer.receivedQuotaChecks.length, 2));

      // Client socket is closed.
      assert(socket.destroyed);

      assert.deepEqual(controlServer.receivedQuotaChecks, [
        {
